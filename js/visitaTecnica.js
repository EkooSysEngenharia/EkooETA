...
export async function montarNovaVisitaVT(container,visitaId=null,preselecionado=null){
 ...
 let fotosVisita=(atual?.fotos||[]).map(f=>({...f}));
 function renderFotos(){
  ...
  area.querySelectorAll("[data-remover-foto]").forEach(b=>b.onclick=()=>{fotosVisita.splice(Number(b.dataset.removerFoto),1);renderFotos()});
 }
 ...
 q("#vtFormVisita").onsubmit=async e=>{
   ...
   const d={usuarioId:uid,clienteId:c.id,clienteNome:nome(c),dataHora:q("#vData").value,responsavel:q("#vResp").value.trim(),local:q("#vLocal").value.trim(),checklist,observacoes:q("#vObs").value.trim(),fotos:fotosVisita,status:"concluida",atualizadoEm:new Date().toISOString()};
...
export async function montarRelatoriosVT(container){
 ...
 const detalhe=container.querySelector("#vtRelDetalhe");container.querySelectorAll("[data-prev]").forEach(b=>b.onclick=()=>{const v=a.find(x=>x.id===b.dataset.prev);if(v){detalhe.innerHTML=vtPreviewRelatorio(v);detalhe.scrollIntoView({behavior:"smooth",block:"start"})}});container.querySelectorAll("[data-pdf]").forEach(b=>b.onclick=()=>{const v=a.find(x=>x.id===b.dataset.pdf);if(v)vtGerarPDF(v)});container.querySelectorAll("[data-xls]").forEach(b=>b.onclick=()=>{const v=a.find(x=>x.id===b.dataset.xls);if(v)vtGerarExcel(v)});
}
